...
void PmergeMe::sort_chains(std::vector<std::pair<size_t *, size_t *> > &chain) {
    ...
    sub_chain.reserve(chain.size() / 2);
    for (size_t i = 0; i < chain.size() / 2; i++) {
        if (chain[2 * i].second <= chain[2 * i + 1].second) {
            sub_chain.push_back(std::make_pair(chain[2 * i].second, chain[2 * i + 1].second));
        }
        else {
            sub_chain.push_back(std::make_pair(chain[2 * i + 1].second, chain[2 * i].second));
        }
    }
            ...
            res.insert(
               std::lower_bound(
                   ...
                   res.begin() +
                       static_cast<std::vector<size_t>::difference_type>(
                           1),
                   chain[i], PmergeMe::PtrChainLargerCmp()),
               chain[i]);
        }
    }

    size_t last_j = 0;
    for (size_t i = 1; PmergeMe::get_nth_jacobsthal(i) <= sub_chain.size();
         i++) {
        last_j = PmergeMe::get_nth_jacobsthal(i);
        for (size_t j = last_j; j > PmergeMe::get_nth_jacobsthal(i - 1); j--) {
    ...
    for (size_t i = sub_chain.size(); i > last_j; i--) {
        for (size_t k = 0; k < chain.size(); ++k) {
            if (chain[k].second == sub_chain[i - 1].first) {
                res.insert(
                    std::lower_bound(
                        res.begin(),
                        res.begin() +
...
//             res.insert(
//                 std::lower_bound(res.begin(), ++res.begin(), *it, VecChainLargerCmp()),
//                 *it);
//             break;
//         }
//     }
...
//                     }
//                     res.insert(
//                         std::lower_bound(res.begin(), kt, *it, VecChainLargerCmp()),
//                         *it);
//                     break;
//                 }
//             }
...
//                      k++, ++kt) {
//                 }
//                 res.insert(std::lower_bound(res.begin(), kt, *it, VecChainLargerCmp()),
//                            *it);
//                 break;
//             }
//         }
...
//     if (chain.size() % 2 == 1)
//         res.insert(
//             std::lower_bound(res.begin(), res.end(), chain.back(), VecChainLargerCmp()),
//             chain.back());
//     chain.swap(res);
// }

bool PmergeMe::PtrChainLargerCmp::operator()(const std::pair<size_t *, size_t *> &a,
